...
| Metrics and alerts | prometheus-client + OpenMetrics + Prometheus rules + Grafana | Explicit gateway-owned bounded registry plus a validated backend-neutral recording/alert/dashboard starter pack |
...
- **Prometheus operations example** (`deploy/examples/observability`): private
  ...
  operator runbook. A repository-authoritative Grafana OSS dashboard provisions
  20 aggregate operations panels and a private Prometheus datasource with no
  host ports or embedded credentials. Thresholds remain proposals until
  calibrated against a named deployment and workload; alert routing, durable
  monitoring storage, and public exposure are not included.
...
- **HTTP API** (`api.rs`, :8932):
  ...
  - `POST /api/v1/sessions` — create a persistent session resource with optional project/template/context/network bindings and optional capability restrictions
