...
- **HTTP API** (`api.rs`, :8932):
  ...
  - `POST /api/v1/projects` — create an owner-scoped project with labels, state, quota metadata, usage-budget enforcement mode, template/egress/extension/reusable-context policy bindings, browser upload/download, session-file binding, manual-recording policy switches, and usage counters
