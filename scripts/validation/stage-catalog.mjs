...
export class ValidationStageCatalog {
  ...
  #buildFastStages(root) {
    const stages = [
      this.#node('validation-tool-tests', 'Test validation tooling', root, [
        ...
        'scripts/ci/compose-evidence-aggregation.test.mjs',
        'scripts/ci/compose-evidence-artifacts.test.mjs',
        ...
        'scripts/observability/grafana-api-probe.test.mjs',
        'scripts/observability/grafana-dashboard-contract.test.mjs',
        'scripts/observability/grafana-validation-stack.test.mjs',
        ...
        'code/web/bpane-client/scripts/admin-smoke-lib.test.mjs',
      ...
      this.#node('production-security-baseline', 'Validate production security baseline', root,
        ['scripts/check-production-security-baseline.mjs'], 180),
      this.#node('single-node-preflight', 'Validate single-node deployment profile', root,
        ['scripts/check-single-node-deployment.mjs', '--repository-fixture'], 180),
  ...
  #buildComposeStages(root) {
    ...
    return [
      ...
      this.#node('observability-grafana-dashboard',
