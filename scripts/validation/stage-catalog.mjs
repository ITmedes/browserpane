...
export class ValidationStageCatalog {
  ...
  select(profile, requestedIds = []) {
    const available =
      profile === 'all' ? [...this.#fastStages, ...this.#composeStages] : this.forProfile(profile);
  ...
  #buildFastStages(root) {
    const stages = [
      ...
      this.#node(
        'repository-baseline',
        'Validate canonical paths and manifests',
        root,
        ['scripts/check-repository-baseline.mjs'],
        120,
      ),
      this.#node(
        'repository-documents',
        'Validate Markdown, YAML, and workflows',
        root,
      ...
      this.#node(
        'dependency-safety',
        'Audit all committed dependency locks',
        root,
    ...
    stages.push(
      ...this.#npmPackage(root, 'admin-auth', 'code/web/bpane-admin-auth', [
        'check',
        'test',
        'test:coverage',
        'build',
    ...
    stages.push(
      ...this.#npmPackage(root, 'admin', 'code/web/bpane-admin', ['check', 'test', 'build']),
    );
    stages.push(
      ...this.#npmPackage(root, 'admin-new', 'code/web/bpane-admin-unified', [
        'check',
        'test',
        'test:coverage',
        'build',
    ...
    stages.push(
      ...this.#npmPackage(root, 'client', 'code/web/bpane-client', [
        'check',
        'test',
        'test:coverage',
        'build',
    ...
    stages.push(
      ...this.#npmPackage(root, 'mcp-bridge', 'code/integrations/mcp-bridge', ['test', 'build']),
    );
    stages.push(
      ...this.#npmPackage(root, 'recording-worker', 'code/integrations/recording-worker', [
        'build',
      ]),
    );
    stages.push(
      ...this.#npmPackage(root, 'workflow-worker', 'code/integrations/workflow-worker', ['build']),
    );
    stages.push(
      ...this.#npmPackage(root, 'openapi', 'scripts/openapi', ['test', 'check', 'compatibility']),
    );
    stages.push(
      this.#node(
        'egress-observer-check',
        'Parse egress observer example',
        root,
        ['--check', 'deploy/examples/egress-observer/egress-usage-reporter.mjs'],
        120,
      ),
    );
    stages.push(
      this.#node(
        'egress-observer-tests',
        'Test egress observer example',
        root,
        ['--test', 'deploy/examples/egress-observer/egress-usage-reporter.test.mjs'],
        180,
      ),
    );
    return stages;
  ...
  #buildComposeStages(root) {
    ...
    return [
      this.#stage(
        'compose-gateway-api',
        'Run gateway compose API suites',
        'bash',
      ...
      this.#npmSmoke(
        ...
        'Smoke compatibility admin session',
        client,
        'smoke:admin-session',
      ),
      ...
      this.#npmSmoke(
        'compose-mcp',
        'Smoke session-scoped MCP endpoint',
        client,
