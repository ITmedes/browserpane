    ...
    >
      ...
      <span class="grid min-w-0 gap-1">
        ...
        <span class="min-w-0 truncate text-xs text-admin-ink/52">
          <span data-testid="session-row-template">{session.template}</span> | <span data-testid="session-row-browser-context">{session.browserContext}</span> | {session.mcpDelegation} | {session.labels} | updated {session.updatedAt}
        </span>
      </span>
