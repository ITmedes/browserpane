...
export class AuthConfigMapper {
  static toAuthConfig(payload: unknown): AuthConfig {
    ...
    const exampleUser = this.#toExampleUser(object.exampleUser);
    const mcpBridge = this.#toMcpBridge(object.mcpBridge);
    return {
      mode,
  ...
  static #toExampleUser(value: unknown): AuthExampleUser | undefined {
  ...
  static #toMcpBridge(value: unknown): McpBridgeConfig | undefined {
    if (value === undefined || value === null) {
      return undefined;
    }
