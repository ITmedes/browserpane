...
describe('buildProjectInspectorModel', () => {
  it('formats metadata, usage, quotas, policy, and alerts', () => {
    ...
    expect(model.sections.map((section) => section.title)).toEqual(['Metadata', 'Usage', 'Quotas', 'Policy']);
    expect(model.sections[1]?.rows).toContainEqual({
      label: 'Active sessions',
      value: '1 / 4',
    });
    expect(model.sections[3]?.rows).toContainEqual({
