...
export type SessionCatalogErrorCode = AdminApiRequestErrorCode;
...
export class SessionCatalogError extends AdminApiRequestError {
  constructor(
    message: string,
...
export class SessionCatalogClient {
  ...
  constructor(options: SessionCatalogClientOptions) {
    ...
    this.#api = new AuthenticatedApiClient({
      ...
      errorFactory: (failure) => new SessionCatalogError(
        formatAdminApiRequestError('Session catalog request', failure),
        failure.code,
        failure.status,
        failure,
      ),
