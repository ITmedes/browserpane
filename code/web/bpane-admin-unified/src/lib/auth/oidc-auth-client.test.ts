...
describe('OidcAuthClient', () => {
  it('builds a PKCE authorization URL and stores login state for the new admin route', async () => {
    const storage = new MemoryStorage();
    const client = newClient(storage, jsonFetch(METADATA));

    const loginUrl = await client.buildLoginUrl(new URL('http://localhost:8080/admin-new/projects?x=1'));
    const url = new URL(loginUrl);

    expect(url.origin).toBe('http://localhost:8091');
    expect(url.searchParams.get('client_id')).toBe('bpane-web');
    expect(url.searchParams.get('redirect_uri')).toBe('http://localhost:8080/admin-new/projects?x=1');
    expect(url.searchParams.get('code_challenge_method')).toBe('S256');
    expect(storage.getItem('bpane.admin.auth.pkce.v1')).toContain('verifier');
  });

  it('exchanges an authorization code and cleans Keycloak callback parameters', async () => {
    const storage = new MemoryStorage();
    ...
    const loginUrl = await client.buildLoginUrl(new URL('http://localhost:8080/admin-new/projects?project=abc'));
    const returnedState = new URL(loginUrl).searchParams.get('state') ?? '';

    const completion = await client.completeLoginIfNeeded(
      new URL(`http://localhost:8080/admin-new/projects?project=abc&code=login-code&state=${returnedState}&session_state=kc`),
    );

    expect(completion).toEqual({
      completed: true,
      cleanUrl: 'http://localhost:8080/admin-new/projects?project=abc',
    });
  ...
  it('builds a Keycloak logout URL and clears local token state', async () => {
    ...
    const logoutUrl = await client.buildLogoutUrl(new URL('http://localhost:8080/admin-new/projects?code=old'));

    expect(logoutUrl).toContain('/protocol/openid-connect/logout');
    expect(logoutUrl).toContain('post_logout_redirect_uri=http%3A%2F%2Flocalhost%3A8080%2Fadmin-new%2Fprojects');
