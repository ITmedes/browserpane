...
export function toWorkflowEventSubscriptionListResponse(
  payload: unknown,
): WorkflowEventSubscriptionListResponse {
  const object = expectRecord(payload, 'workflow event subscription list response');
  return {
    subscriptions: expectArray(object.subscriptions, 'workflow event subscriptions').map(
      toWorkflowEventSubscriptionResource,
    ),
  };
}

export function toWorkflowEventSubscriptionResource(
  value: unknown,
): WorkflowEventSubscriptionResource {
  ...
  return {
    ...
    event_types: expectArray(object.event_types, 'workflow event subscription event_types').map(
      (eventType) => expectString(eventType, 'workflow event subscription event type'),
    ),
    has_signing_secret: expectBoolean(
      object.has_signing_secret,
      'workflow event subscription has_signing_secret',
    ),
    deliveries_path: expectString(
...
export function toWorkflowEventDeliveryListResponse(
  payload: unknown,
): WorkflowEventDeliveryListResponse {
  const object = expectRecord(payload, 'workflow event delivery list response');
  return {
    deliveries: expectArray(object.deliveries, 'workflow event deliveries').map(
      toWorkflowEventDeliveryResource,
    ),
  };
}
...
export function toWorkflowEventDeliveryResource(value: unknown): WorkflowEventDeliveryResource {
  ...
  return {
    ...
    next_attempt_at:
      optionalString(object.next_attempt_at, 'workflow event delivery next_attempt_at') ?? null,
    last_attempt_at:
      optionalString(object.last_attempt_at, 'workflow event delivery last_attempt_at') ?? null,
    delivered_at:
      optionalString(object.delivered_at, 'workflow event delivery delivered_at') ?? null,
    last_response_status:
      optionalNumber(object.last_response_status, 'workflow event delivery last_response_status') ??
      null,
    last_error: optionalString(object.last_error, 'workflow event delivery last_error') ?? null,
    payload: object.payload,
    attempts: expectArray(object.attempts, 'workflow event delivery attempts').map(
      toWorkflowEventDeliveryAttemptResource,
    ),
...
function toWorkflowEventDeliveryAttemptResource(
  value: unknown,
): WorkflowEventDeliveryAttemptResource {
  ...
  return {
    ...
    response_status:
      optionalNumber(object.response_status, 'workflow event delivery attempt response_status') ??
      null,
...
function optionalNumber(value: unknown, label: string): number | null | undefined {
  return value === undefined || value === null ? value : expectNumber(value, label);
}
