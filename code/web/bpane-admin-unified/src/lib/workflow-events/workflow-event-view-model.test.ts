...
} from './workflow-event-view-model';
import type {
...
describe('workflow event view model', () => {
  ...
  it('validates subscription input and keeps signing secret write-only in the request', () => {
    ...
    Object.assign(draft, {
      ...
      targetUrl: 'https://events.example/hook',
      eventTypesText: 'workflow_run.created,workflow_run.failed',
      signingSecret: 'secret-value',
    });
    expect(validateWorkflowEventSubscriptionDraft(draft).request).toEqual({
      name: 'Workflow events',
    ...
    Object.assign(draft, {
      ...
      eventTypesText: 'bad event',
      signingSecret: '',
    });
    expect(validateWorkflowEventSubscriptionDraft(draft)).toMatchObject({
      valid: false,
      fieldErrors: {
        targetUrl: expect.any(Array),
        eventTypes: expect.any(Array),
        signingSecret: expect.any(Array),
      },
    });
  });
  it('summarizes failed delivery diagnostics and attempts', () => {
    ...
    expect(model.rows[0]?.attemptsDetail[0]).toMatchObject({
      label: 'Attempt 1',
      response: 'HTTP 503',
    });
  });
...
function subscription(): WorkflowEventSubscriptionResource {
  return {
    id: 'subscription-1',
    name: 'Workflow events',
    target_url: 'https://events.example/hook',
...
function delivery(): WorkflowEventDeliveryResource {
  return {
    id: 'delivery-1',
