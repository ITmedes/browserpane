...
export type WorkflowEventDetailLoadState =
  ...
  | {
      readonly status: 'ready';
      readonly subscription: WorkflowEventSubscriptionResource;
      readonly deliveries: readonly WorkflowEventDeliveryResource[];
    };
export type WorkflowEventSubscriptionDraft = {
  name: string;
  targetUrl: string;
  eventTypesText: string;
  signingSecret: string;
};
export type WorkflowEventSubscriptionValidation = {
  readonly valid: boolean;
  readonly request: CreateWorkflowEventSubscriptionRequest | null;
  readonly fieldErrors: Readonly<Record<string, readonly string[]>>;
};

export function buildWorkflowEventOverviewModel(
  subscriptions: readonly WorkflowEventSubscriptionResource[],
) {
  return {
    metrics: [
      ...
      metric(
        'signed',
        'Signed',
        subscriptions.filter((subscription) => subscription.has_signing_secret).length,
      ),
      metric(
        'wildcard',
        'Wildcard filters',
        subscriptions.filter((subscription) =>
          subscription.event_types.some((type) => type.endsWith('.*')),
        ).length,
      ),
      metric(
        'event-types',
        'Event filters',
        subscriptions.reduce((total, subscription) => total + subscription.event_types.length, 0),
      ),
    ],
...
export function workflowEventSubscriptionMatchesSearch(
  row: ReturnType<typeof buildWorkflowEventOverviewModel>['rows'][number],
  query: string,
): boolean {
  if (!query) return true;
  return [row.id, row.name, row.targetUrl, row.eventTypes, row.signing, row.updatedAt].some(
...
export function validateWorkflowEventSubscriptionDraft(
  draft: WorkflowEventSubscriptionDraft,
): WorkflowEventSubscriptionValidation {
  ...
  if (!isHttpUrl(targetUrl))
    fieldErrors.targetUrl = ['Target URL must be an absolute HTTP or HTTPS URL.'];
  if (eventTypes.length === 0) fieldErrors.eventTypes = ['At least one event type is required.'];
  else if (eventTypes.some((eventType) => eventType.includes(' ')))
    fieldErrors.eventTypes = ['Event types must not contain spaces.'];
  ...
  return {
    ...
    fieldErrors,
    request: valid
      ? { name, target_url: targetUrl, event_types: eventTypes, signing_secret: signingSecret }
      : null,
  };
}

export function buildWorkflowEventDeliverySummary(
  deliveries: readonly WorkflowEventDeliveryResource[],
) {
  return {
    metrics: [
      ...
      metric(
        ...
        'Pending / delivering',
        deliveries.filter(
          (delivery) => delivery.state === 'pending' || delivery.state === 'delivering',
        ).length,
      ),
      metric(
        'failed',
...
export function workflowEventDeliveryRow(delivery: WorkflowEventDeliveryResource) {
  return {
    ...
    response: delivery.last_response_status
...
function deliveryStateLabel(state: WorkflowEventDeliveryState): string {
  return state.charAt(0).toUpperCase() + state.slice(1);
}
function deliveryStateTone(state: WorkflowEventDeliveryState): ProjectTone {
  if (state === 'delivered') return 'success';
  if (state === 'failed') return 'danger';
  if (state === 'pending' || state === 'delivering') return 'warning';
  return 'neutral';
}
function metric(key: string, label: string, value: number) {
  return { label, value: String(value), testId: `workflow-events-metric-${key}` };
}
