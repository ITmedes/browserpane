...
describe('WorkflowEventCatalogClient', () => {
  it('manages subscriptions and loads delivery diagnostics through authenticated requests', async () => {
    const fetchImpl = vi.fn<typeof fetch>(async (input, init) => {
      ...
      if (url.endsWith('/workflow-event-subscriptions') && init?.method === 'GET')
        return jsonResponse({ subscriptions: [subscriptionPayload()] }, 200);
    ...
    const client = new WorkflowEventCatalogClient({
    ...
    const request = {
      ...
      signing_secret: 'write-only-secret',
    };
