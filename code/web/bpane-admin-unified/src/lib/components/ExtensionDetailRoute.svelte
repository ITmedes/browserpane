<script lang="ts">
  ...
  onMount(() => {
    ...
    if (!extensionId) {
      extensionState = {
        status: 'error',
        extensionId: 'unknown',
        message: 'Extension id is missing from the current route.',
      };
      return;
    }
  ...
  async function loadExtension(extensionId: string): Promise<void> {
    ...
    try {
      ...
      extensionState = {
        status: 'error',
        extensionId,
        message: adminErrorMessage(error, 'Unexpected extension detail error.'),
      };
    }
  }
  ...
  async function refreshExtension(): Promise<void> {
    ...
    try {
      ...
      actionState = {
        status: 'error',
        message: adminErrorMessage(error, 'Extension refresh failed.'),
      };
    }
  }
  ...
  async function setEnabled(enabled: boolean): Promise<void> {
    ...
    actionState = {
      status: 'running',
      label: enabled ? 'Enabling extension...' : 'Disabling extension...',
    };
    try {
      extensionState = {
        status: 'ready',
        extension: await client().setExtensionEnabled(extensionId, enabled),
      };
      actionState = {
        status: 'success',
      ...
      actionState = {
        status: 'error',
        message: adminErrorMessage(error, 'Extension state change failed.'),
      };
    }
  }
  ...
  async function publishVersion(request: CreateExtensionVersionRequest): Promise<void> {
    ...
    actionState = {
      status: 'running',
      label: `Publishing extension version ${request.version}...`,
    };
    try {
      ...
      extensionState = {
        status: 'ready',
        extension: await extensionClient.getExtension(extensionId),
      };
      actionState = {
        status: 'success',
        message: `Extension version ${request.version} published.`,
      };
    } catch (error) {
      actionState = {
        status: 'error',
        message: adminErrorMessage(error, 'Extension version publication failed.'),
      };
    }
  }
  ...
  function activeExtensionId(): string | null {
    ...
    if (extensionState.status === 'loading' || extensionState.status === 'error')
      return extensionState.extensionId;
    return null;
  }
</script>
...
<div
  ...
  data-testid="extension-detail-route"
>
  <header class="border-b border-admin-border pb-4">
    <a
      ...
      data-testid="extension-detail-back"
  ...
  {#if extensionState.status === 'error'}
    <AdminMessage
      tone="error"
      title="Extension detail unavailable"
      message={extensionState.message}
      testId="extension-detail-error"
    />
  {:else}
    <ExtensionInspector
