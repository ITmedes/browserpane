...
beforeEach(() => window.history.replaceState(null, '', 'http://localhost:3000/admin-new/extensions/extension-1'));
  ...
  it('loads, versions, and changes extension state', async () => {
    ...
    const fetchImpl = vi.fn<typeof fetch>(async (input, init) => {
      ...
      if (url.endsWith('/versions')) {
        latestVersion = '1.0.0';
        return jsonResponse({ id: 'version-1', extension_definition_id: 'extension-1', version: '1.0.0', install_path: '/opt/extensions/login', created_at: '2026-08-07T09:30:00.000Z' }, 201);
      }
      if (url.endsWith('/disable')) {
        enabled = false;
        return jsonResponse(extensionPayload(enabled, latestVersion), 200);
      }
      if (init?.method === 'GET') return jsonResponse(extensionPayload(enabled, latestVersion), 200);
      return new Response('not found', { status: 404 });
    });
    vi.stubGlobal('fetch', fetchImpl);
    const target = renderComponent(ExtensionDetailRoute, { authContext: authContext() });
    await vi.waitFor(() => expect(byTestId(target, 'extension-detail-name').textContent).toContain('Login helper'));
    ...
    await vi.waitFor(() => expect(byTestId(target, 'extension-action-success').textContent).toContain('published'));
    ...
    await vi.waitFor(() => expect(byTestId(target, 'extension-detail-state').textContent).toContain('Disabled'));
  });
});
...
function extensionPayload(enabled: boolean, latestVersion: string | null) {
  return { id: 'extension-1', name: 'Login helper', description: null, enabled, latest_version: latestVersion, labels: {}, created_at: '2026-08-07T08:00:00.000Z', updated_at: '2026-08-07T09:00:00.000Z' };
}
