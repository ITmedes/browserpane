...
describe('CredentialBindingCreateForm', () => {
  it('validates secret JSON, origins, and labels near their controls', async () => {
    ...
    expect(byTestId(target, 'credential-binding-origins-error').textContent).toContain(
      'HTTP or HTTPS',
    );
    expect(byTestId(target, 'credential-binding-secret-payload-error').textContent).toContain(
      'JSON object',
    );
  ...
  it('submits project-scoped write-only payloads and TOTP metadata', async () => {
    ...
    await input(
      target,
      'credential-binding-secret-payload',
      '{"secret":"sensitive","selector":"#otp"}',
    );
    ...
    expect(onSave).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        secret_payload: { secret: 'sensitive', selector: '#otp' },
        totp: expect.objectContaining({ issuer: 'BrowserPane', digits: 6 }),
      }),
    );
  });
  ...
  it('supports opaque provider references without a secret payload', async () => {
    const onSave = vi.fn();
    const target = renderComponent(CredentialBindingCreateForm, { onSave });
    await input(target, 'credential-binding-name', 'Existing');
    byTestId(target, 'credential-binding-secret-reference-mode').click();
    await tick();
    await input(target, 'credential-binding-external-ref', 'secret/data/existing');
    byTestId(target, 'credential-binding-create-submit').click();
    expect(onSave).toHaveBeenCalledWith(
      expect.objectContaining({ external_ref: 'secret/data/existing' }),
    );
...
async function select(target: HTMLElement, testId: string, value: string) {
