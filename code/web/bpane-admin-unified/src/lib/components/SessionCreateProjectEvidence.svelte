<script lang="ts">
  ...
  type SessionCreateProjectEvidenceProps = {
    readonly project: ProjectResource;
  };

  let { project }: SessionCreateProjectEvidenceProps = $props();
  ...
  const queuedSessions = $derived(metric('queued_sessions'));
...
<section class="mt-4 border-t border-admin-border pt-4" data-testid="session-create-project-governance">
  <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
    <div class="min-w-0">
      ...
      <p class="m-0 mt-1 text-xs leading-5 text-admin-muted">
        Gateway admission remains authoritative if policy or capacity changes before submission.
      </p>
    </div>
    <a class="text-xs font-semibold text-admin-accent hover:underline" href={`/admin-new/projects/${encodeURIComponent(project.id)}`}>
      Project details
    </a>
  </div>
  ...
  <div class="mt-3 flex flex-wrap gap-x-5 gap-y-2 text-xs text-admin-muted">
    ...
    {#if queuedSessions}<span>Queued sessions: <strong class="text-admin-ink">{queuedSessions.current}</strong></span>{/if}
    <span>Budgets: <strong class="text-admin-ink">{usage.enforcement}</strong></span>
  </div>

  {#if usage.alerts.length > 0}
    <div class="mt-3 grid gap-2" data-testid="session-create-project-alerts">
