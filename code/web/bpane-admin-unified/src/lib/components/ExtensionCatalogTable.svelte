<script lang="ts">
  ...
  const lenses = $derived([
    ...
    { id: 'enabled' as const, label: 'Enabled', count: model.rows.filter((row) => row.status === 'Enabled').length },
    { id: 'disabled' as const, label: 'Disabled', count: model.rows.filter((row) => row.status === 'Disabled').length },
    { id: 'versioned' as const, label: 'Versioned', count: model.rows.filter((row) => row.latestVersion !== 'No version').length },
    { id: 'unversioned' as const, label: 'No version', count: model.rows.filter((row) => row.latestVersion === 'No version').length },
  ]);
  ...
  function filterRows(
    ...
    return rows.filter((row) => matchesLens(row, selectedLens) && extensionMatchesSearch(row, normalizedQuery));
  }
...
<section class="min-h-0 min-w-0 rounded-md border border-admin-border bg-admin-panel" data-testid="extensions-list">
  <div class="flex flex-col gap-3 border-b border-admin-border px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
    <div class="min-w-0">
      ...
      <p class="m-0 mt-1 text-xs text-admin-muted">Deployment-managed unpacked browser extensions approved for future sessions and workflows.</p>
    </div>
    <span class="text-xs text-admin-muted" data-testid="extensions-list-count">{visibleRows.length} of {model.rows.length}</span>
  </div>
    ...
    <div class="flex min-w-0 flex-wrap items-center px-4">
      {#each lenses as definition}
        <button
          ...
          onclick={() => { lens = definition.id; }}
          data-testid={`extensions-lens-${definition.id}`}
        >
          <span>{definition.label}</span>
          <span class="rounded border border-admin-border bg-admin-soft px-1.5 py-0.5 text-[11px] font-semibold text-admin-muted">{definition.count}</span>
        </button>
      {/each}
    </div>
    ...
    <table class="w-full min-w-[860px] border-collapse">
      <thead class="sticky top-0 z-10 bg-admin-soft">
        <tr class="border-b border-admin-border">
          <th class="px-4 py-2 text-left text-xs font-bold uppercase text-admin-muted" scope="col">Extension</th>
          <th class="px-3 py-2 text-left text-xs font-bold uppercase text-admin-muted" scope="col">State</th>
          <th class="px-3 py-2 text-left text-xs font-bold uppercase text-admin-muted" scope="col">Latest version</th>
      ...
      <tbody>
        {#if visibleRows.length === 0}
          <tr><td class="px-4 py-14 text-center text-sm text-admin-muted" colspan="6" data-testid="extensions-filter-empty">No extensions match the current filters.</td></tr>
        {:else}
          {#each visibleRows as row}
            <tr class="border-b border-admin-border last:border-b-0 hover:bg-admin-soft" data-testid="extensions-list-row">
              ...
              <td class="px-3 py-3"><span class={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ring-1 ${projectToneClass(row.statusTone)}`}>{row.status}</span></td>
              <td class="px-3 py-3"><span class={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ring-1 ${projectToneClass(row.versionTone)}`}>{row.latestVersion}</span></td>
              <td class="max-w-[220px] px-3 py-3 text-xs text-admin-muted"><span class="line-clamp-2">{row.labels}</span></td>
              <td class="px-3 py-3 text-xs text-admin-muted">{row.updatedAt}</td>
              <td class="px-4 py-3 text-right">
                <a class="inline-flex h-8 items-center rounded-md border border-admin-border bg-admin-panel px-3 text-xs font-semibold text-admin-ink hover:bg-admin-soft" href={`/admin-new/extensions/${encodeURIComponent(row.id)}`} data-testid="extensions-detail-link">Details</a>
              </td>
            </tr>
          {/each}
        {/if}
