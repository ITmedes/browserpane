...
<section class="rounded-md border border-admin-border bg-admin-soft/50 p-4" data-testid="egress-profile-status-summary">
  ...
  <dl class="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
    {#each model.items as item}
      <div class="rounded-md border border-admin-border bg-admin-panel p-3">
        ...
        <dd class={`m-0 mt-1 inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ring-1 ${projectToneClass(item.tone)}`}>
          {item.value}
        </dd>
      </div>
    {/each}
