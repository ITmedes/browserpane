...
beforeEach(() =>
  window.history.replaceState(null, '', 'http://localhost:3000/admin-new/extensions/new'),
);
afterEach(async () => {
  vi.unstubAllGlobals();
...
describe('ExtensionCreateRoute', () => {
  it('creates an extension and delegates detail navigation', async () => {
    ...
    const target = renderComponent(ExtensionCreateRoute, {
    ...
    await vi.waitFor(() =>
      expect(navigateToExtension).toHaveBeenCalledWith(
        expect.objectContaining({ id: 'extension-1' }),
      ),
    );
    expect(fetchImpl.mock.calls[0]?.[1]?.body).toBe(
      JSON.stringify({ name: 'Login helper', description: null, labels: {} }),
    );
  });
  ...
  it('shows API validation failures', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn<typeof fetch>(async () => jsonResponse({ error: 'extension rejected' }, 400)),
    );
    const target = renderComponent(ExtensionCreateRoute, { authContext: authContext() });
    await input(target, 'extension-create-name', 'Login helper');
    byTestId(target, 'extension-create-submit').click();
    await vi.waitFor(() =>
      expect(byTestId(target, 'extension-create-error').textContent).toContain(
        'extension rejected',
      ),
    );
  });
