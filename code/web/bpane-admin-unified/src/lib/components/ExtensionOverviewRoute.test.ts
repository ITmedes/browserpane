...
beforeEach(() =>
  window.history.replaceState(null, '', 'http://localhost:3000/admin-new/extensions'),
);
afterEach(async () => {
  vi.unstubAllGlobals();
...
describe('ExtensionOverviewRoute', () => {
  ...
  it('delegates authentication failures', async () => {
    ...
    const target = renderComponent(ExtensionOverviewRoute, {
      authContext: authContext(onAuthenticationFailure),
    });
    await vi.waitFor(() =>
      expect(byTestId(target, 'extensions-error')).toBeInstanceOf(HTMLElement),
    );
