<script lang="ts">
  ...
  import {
    ...
    type CredentialBindingOverviewLoadState,
  ...
  let {
    ...
    readonly state: CredentialBindingOverviewLoadState;
    readonly onRefresh?: () => void | Promise<void>;
  } = $props();
  const model = $derived(
    loadState.status === 'ready' ? buildCredentialBindingOverviewModel(loadState.bindings) : null,
  );
</script>

<div
  class="mx-auto flex min-h-full w-full max-w-[1440px] flex-col gap-5 px-4 py-5 sm:px-6 lg:px-8"
  data-testid="credential-bindings-overview"
>
  <header
  ...
  >
    <div>
      ...
      <h1 class="m-0 mt-1 text-2xl font-semibold text-admin-ink">Credential bindings</h1>
    </div>
    <div class="flex flex-wrap gap-2">
      <a
        ...
        href="/admin-new/credential-bindings/new"
        data-testid="credential-bindings-new-link"
        ><Plus size={16} strokeWidth={1.9} /><span>New binding</span></a
      ><button
        ...
        data-testid="credential-bindings-refresh-button"
  ...
  {#if loadState.status === 'loading'}
    <section
      ...
      data-testid="credential-bindings-loading"
    >
      <AdminMessage
        tone="loading"
        title="Loading credential bindings"
        message="Safe binding metadata is being refreshed from the control API."
      />
    </section>
  {:else if loadState.status === 'error'}
    <AdminMessage
      tone="error"
      title="Credential binding catalog unavailable"
      message={loadState.message}
      testId="credential-bindings-error"
    />
  {:else if loadState.bindings.length === 0}
    <section
      ...
      data-testid="credential-bindings-empty"
    >
      Credential binding catalog is empty.
    </section>
  {:else if model}
    <section
      class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"
      aria-label="Credential binding metrics"
    >
