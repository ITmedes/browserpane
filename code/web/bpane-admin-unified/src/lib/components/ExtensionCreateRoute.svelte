<script lang="ts">
  ...
  import type { CreateExtensionDefinitionRequest, ExtensionDefinitionResource } from '$lib/extensions/extension-types';
  import ActionFeedback from './ActionFeedback.svelte';
  import ExtensionCreateForm from './ExtensionCreateForm.svelte';

  let {
    authContext,
    navigateToExtension = async (extension: ExtensionDefinitionResource) => goto(`/admin-new/extensions/${encodeURIComponent(extension.id)}`),
  }: {
  ...
  async function createExtension(request: CreateExtensionDefinitionRequest): Promise<void> {
    ...
    try {
      ...
      actionState = { status: 'error', message: adminErrorMessage(error, 'Extension creation failed.') };
    }
  }
</script>

<div class="mx-auto flex min-h-full w-full max-w-[1180px] flex-col gap-5 px-4 py-5 sm:px-6 lg:px-8" data-testid="extension-create-route">
  <header class="border-b border-admin-border pb-4">
    <a class="inline-flex items-center gap-2 text-sm font-medium text-admin-muted hover:text-admin-ink" href="/admin-new/extensions" data-testid="extension-create-back"><ArrowLeft size={16} strokeWidth={1.8} /><span>Approved extensions</span></a>
  ...
  <ActionFeedback state={actionState} successTitle="Extension action completed" errorTitle="Extension action failed" successTestId="extension-create-success" errorTestId="extension-create-error" runningTestId="extension-create-running" />
