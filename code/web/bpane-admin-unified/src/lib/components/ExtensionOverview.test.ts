...
describe('ExtensionOverview', () => {
  it('renders loading, error, empty, and ready states', async () => {
    ...
    expect(byTestId(target, 'extensions-loading').textContent).toContain(
      'Loading approved extensions',
    );

    await cleanupRenderedComponents();
    target = renderComponent(ExtensionOverview, {
    ...
    await cleanupRenderedComponents();
    target = renderComponent(ExtensionOverview, {
      state: { status: 'ready', extensions: [extension()] },
    });
  ...
  it('delegates refresh', () => {
    ...
    const target = renderComponent(ExtensionOverview, {
      state: { status: 'ready', extensions: [extension()] },
      onRefresh,
    });
