<script lang="ts">
  ...
  import {
    ...
    type ExtensionOverviewLoadState,
  ...
  const model = $derived(
    loadState.status === 'ready' ? buildExtensionOverviewModel(loadState.extensions) : null,
  );
</script>

<div
  class="mx-auto flex min-h-full w-full max-w-[1440px] flex-col gap-5 px-4 py-5 sm:px-6 lg:px-8"
  data-testid="extensions-overview"
>
  <header
  ...
  >
    ...
    <div class="flex flex-wrap gap-2">
      <a
        ...
        href="/admin-new/extensions/new"
        data-testid="extensions-new-link"
      >
  ...
  {#if loadState.status === 'loading'}
    ...
    >
      <AdminMessage
        tone="loading"
        title="Loading approved extensions"
        message="The extension catalog is being refreshed from the control API."
      />
    </section>
  ...
  {:else if loadState.extensions.length === 0}
    ...
    >
      Approved extension catalog is empty.
    </section>
  {:else if model}
