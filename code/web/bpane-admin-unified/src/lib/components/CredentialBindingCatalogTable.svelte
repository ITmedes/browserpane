<script lang="ts">
  ...
  const lenses = $derived([
    { id: 'all' as const, label: 'All', count: model.rows.length },
    {
      id: 'owner' as const,
      label: 'Owner',
      count: model.rows.filter((row) => row.scope === 'Owner scoped').length,
    },
    {
      id: 'project' as const,
      label: 'Project',
    ...
    },
    {
      id: 'form' as const,
      label: 'Form fill',
      count: model.rows.filter((row) => row.injectionMode === 'Form fill').length,
    },
    {
      id: 'cookie' as const,
      label: 'Cookie',
      count: model.rows.filter((row) => row.injectionMode === 'Cookie seed').length,
    },
    {
      id: 'storage' as const,
      label: 'Storage',
      count: model.rows.filter((row) => row.injectionMode === 'Storage seed').length,
    },
    {
      id: 'totp' as const,
      label: 'TOTP',
      count: model.rows.filter((row) => row.injectionMode === 'TOTP fill').length,
    },
  ]);

  function filterRows(
    rows: readonly CredentialBindingOverviewRow[],
    selectedLens: BindingLens,
    query: string,
  ) {
    const normalizedQuery = query.trim().toLowerCase();
    return rows.filter(
      (row) =>
        matchesLens(row, selectedLens) && credentialBindingMatchesSearch(row, normalizedQuery),
    );
  }
...
<section
  ...
  data-testid="credential-bindings-list"
>
  <div
    class="flex flex-col gap-3 border-b border-admin-border px-4 py-3 lg:flex-row lg:items-center lg:justify-between"
  >
    <div class="min-w-0">
      <h2 class="m-0 text-sm font-semibold text-admin-ink">Credential binding catalog</h2>
      <p class="m-0 mt-1 text-xs text-admin-muted">
        Write-only secret references available to approved workflow and egress integrations.
      </p>
    </div>
    <span class="text-xs text-admin-muted" data-testid="credential-bindings-list-count"
  ...
  >
    <div class="flex min-w-0 flex-wrap items-center px-4">
      {#each lenses as definition}
        <button
          ...
          data-testid={`credential-bindings-lens-${definition.id}`}
    ...
    <label
      ...
      ><Search size={15} strokeWidth={1.8} /><span class="sr-only">Search credential bindings</span
      ><input
        class="min-w-0 flex-1 border-0 bg-transparent text-sm text-admin-ink outline-none placeholder:text-admin-muted"
        type="search"
        placeholder="Name, scope, origin..."
        bind:value={searchQuery}
        data-testid="credential-bindings-search"
      /></label
    >
  </div>
    ...
    <table class="w-full min-w-[980px] border-collapse">
      <thead class="sticky top-0 z-10 bg-admin-soft"
        ><tr class="border-b border-admin-border"
          ><th class="px-4 py-2 text-left text-xs font-bold uppercase text-admin-muted">Binding</th
          ><th class="px-3 py-2 text-left text-xs font-bold uppercase text-admin-muted">Scope</th
        ...
        {#if visibleRows.length === 0}
          <tr
            ><td
              ...
              colspan="6"
              data-testid="credential-bindings-filter-empty"
              >No credential bindings match the current filters.</td
            ></tr
          >
        {:else}
          {#each visibleRows as row}
            <tr
              ...
              data-testid="credential-bindings-list-row"
            >
              <td class="w-[290px] px-4 py-3"
                ><div class="grid min-w-0">
                  <span class="truncate text-sm font-semibold text-admin-ink">{row.name}</span><span
                    class="mt-1 truncate text-xs text-admin-muted"
                    >{row.provider} · {row.namespace}</span
              ...
              <td class="max-w-[180px] px-3 py-3"
                ><span
                  class={`inline-flex max-w-full rounded-full px-2 py-0.5 text-xs font-semibold ring-1 ${projectToneClass(row.scopeTone)}`}
                  ><span class="truncate">{row.scope}</span></span
                ></td
              >
              <td class="px-3 py-3 text-sm text-admin-ink">{row.injectionMode}</td>
              <td class="max-w-[260px] px-3 py-3 text-xs text-admin-muted"
                ><span class="line-clamp-2">{row.origins}</span></td
              >
              ...
              <td class="px-4 py-3 text-right"
                ><a
                  ...
                  href={`/admin-new/credential-bindings/${encodeURIComponent(row.id)}`}
                  data-testid="credential-bindings-detail-link">Details</a
                ></td
              >
            </tr>
          {/each}
