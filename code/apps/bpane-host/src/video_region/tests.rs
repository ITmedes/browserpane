...
fn editable_hint_holds_within_window() {
    ...
    assert!(
        state.last_hint.is_some(),
        "hint should be held within window"
    );
}
...
#[test]
fn region_committer_clear_drops_pending_and_active() {
    let mut rc = RegionCommitter::new();
    let current = region(0, 0, 640, 360);
    let pending = region(64, 64, 640, 360);
    let next = region(128, 128, 640, 360);
    rc.active = Some(current);
    assert_eq!(rc.commit(Some(pending), 3), Some(current));

    let now = Instant::now();
    rc.clear(now);
    assert!(rc.active.is_none());
    assert_eq!(rc.last_reconfig_at, now);
    assert_eq!(rc.commit(Some(next), 3), Some(next));
}
