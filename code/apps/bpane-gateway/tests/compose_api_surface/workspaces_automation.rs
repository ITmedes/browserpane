...
pub async fn run(harness: &ComposeHarness) -> Result<()> {
    ...
    let fetched_file = harness
        .get_json(&format!("/api/v1/file-workspaces/{workspace_id}/files/{file_id}"))
        .await?;
    ...
    if file_bytes != b"compose gateway e2e workspace bytes" {
        return Err(anyhow!("workspace file content endpoint returned unexpected bytes"));
    }
    ...
    {
        return Err(anyhow!("automation task {task_id} missing from list endpoint"));
    }
    ...
    if fetched_task["executor"] != json!("compose_e2e") {
        return Err(anyhow!("automation task lookup returned the wrong executor"));
    }
    ...
    let automation_headers = map_headers(&[("x-bpane-automation-access-token", &automation_token)])?;
    ...
    if task_events.len() < 2 {
        return Err(anyhow!("automation task events did not capture state transitions"));
    }
    ...
    if task_logs.is_empty() {
        return Err(anyhow!("automation task logs did not capture automation output"));
    }
    ...
    if cancelled["state"] != json!("cancelled") {
        return Err(anyhow!("automation task cancel endpoint did not cancel the task"));
    }

    let deleted_file = harness
        .delete_json(&format!("/api/v1/file-workspaces/{workspace_id}/files/{file_id}"))
        .await?;
