...
| M2 no CSP/security headers | Resolved by #146. Both admin routes use the shared CSP and nginx browser-security header contract with static and live smoke coverage. | `BPANE-00146_ADMIN_AUTH_SECURITY_PLAN.md`, `SECURITY_RUNTIME_ROADMAP.md` |
| H2/H3 admin tokens in `sessionStorage`, no OIDC nonce, no ID-token verification | Resolved by #146. Tokens remain in memory, PKCE state/nonce is bounded per tab, and the certified protocol core validates issuer, audience, expiry, nonce, and signature before claims are displayed. | `BPANE-00146_ADMIN_AUTH_SECURITY_PLAN.md`, `SECURITY_RUNTIME_ROADMAP.md` |
