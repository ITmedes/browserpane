...
Revalidated against current routes and package scripts: 2026-08-21
...
| Workflows | `/admin-new/workflows`, `/admin-new/workflows/[workflow_id]` | Partial | Catalog, detail, source tree/code preview, package/schema JSON publication, exact package/commit/requirement/evidence/compatibility inspection, workflow launch controls, project selector, and project pressure/alert guidance exist. Broader catalog management remains incomplete. |
| Workflow integration endpoints | `/admin-new/workflow-endpoints`, `/admin-new/workflow-endpoints/[project_id]/[endpoint_key]` | Implemented for #172 Phase 0 scope | Project selector/catalog, draft creation, contract/schema/limit detail, activate/disable actions, narrow service-principal grants, machine-token/idempotency invocation guidance, and related workflow-run navigation are route-backed. #240 retains revision promotion/rollback, callbacks, replay, tracing, overload/readiness, and connector diagnostics. |
...
| API coverage | `/admin-new/coverage` | Implemented for #158 scope | All 144 frozen operations can be searched and filtered by family, classification, and auth domain from the generated contract evidence, including the distinct confidential-machine auth class for four Workflow Endpoint routes. |
...
| Step 17: API reference/coverage companion | Copyable API examples, operation classification, OpenAPI link, compatibility separation. | Done for current contract | `/api`, `/coverage`, and `/docs` load strictly validated committed evidence, expose 22 schema-validated examples and 14 separate compatibility surfaces, and are compose-smoke covered. The endpoint task flow uses a machine-token placeholder and canonical idempotency header without rendering a credential. |
