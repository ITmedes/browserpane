...
Revalidated against current routes and package scripts: 2026-08-20
...
| Workflow integration endpoints | Recommended future `/admin-new/workflow-endpoints`, `/new`, `/[endpoint_id]`, `/runs`, `/deliveries` | Missing | `#172` owns the bounded Phase 0 endpoint catalog/detail/grants and polling-run surface. `#240` owns later revision promotion/rollback, callbacks, replay, tracing, overload/readiness, and connector diagnostics. This is not an Admin-New promotion blocker. |
