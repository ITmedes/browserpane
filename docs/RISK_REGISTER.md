...
| R-007 | Protocol version is not enforced and runtime compatibility is not qualified. | Medium | Critical | #175 | #263 publishes the normative v1 contract, numeric registries, typed failures, legacy matrix, and an exhaustive shared 15-vector seed baseline. Runtime codecs/enforcement, expanded vectors, fuzzing, and the real rolling compatibility matrix remain #264-#268. | Production |
